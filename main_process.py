...
def play(size, pit_density, num_wumpus):
    ...
    while True:
        ...
        #prune_by_radius(game.agent, game.agent.size_known) # Pruning KB
        ...
        if flag == False:
            ...
            game.update_score()
            break
        if (len(game.agent.actions) > 30):
            print("Too many actions, stopping the game.")
            break
    return game.point, game.agent.actions

point, action = play(8, 0.1, 2)  # Example call to play the game with a 4x4 map, 20% pit density, and 2 wumpuses
